...
namespace quda {
//Notes:
//FGMResDR does not require large m (and esp. nev), so this will use normal LAPACK routines.
//TODO : 
...
//special types needed for compatibility with QUDA blas:
    ...
    class FGMResDRArgs{
      ...
      public:
       ...
       FGMResDRArgs(int m, int nev) : ritzVecs(VectorSet::Zero(m+1,nev+1)), H(DenseMatrix::Zero(m+1,m)),
   ...
   void ComputeHarmonicRitz(FGMResDRArgs &args)
   {
#ifdef DEFLATEDSOLVER
 ...
 FGMResDR::FGMResDR(DiracMatrix &mat, DiracMatrix &matSloppy, DiracMatrix &matPrecon, SolverParam &param, TimeProfile &profile) :
 ...
 FGMResDR::FGMResDR(DiracMatrix &mat, Solver &K, DiracMatrix &matSloppy, DiracMatrix &matPrecon, SolverParam &param, TimeProfile &profile) :
 ...
 void FGMResDR::UpdateSolution(ColorSpinorField *x, ColorSpinorField *r, bool do_gels)
 {
#ifdef DEFLATEDSOLVER
   FGMResDRArgs &args = *gmresdr_args;
...
int FGMResDR::FlexArnoldiProcedure(const int start_idx, const bool do_givens = false)
 {
   int j = start_idx;
 ...
 void FGMResDR::operator()(ColorSpinorField &x, ColorSpinorField &b)
 {
    ...
    if (!init) {
      ...
      gmresdr_args = new FGMResDRArgs(param.m, param.nev);
...
#else
   errorQuda("Deflated solver was not built.\n");
#endif
